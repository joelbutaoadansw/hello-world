...
console.log('Start')
const sql_query = {
    new_addresses:"SELECT top 50 dbo.Form_AustralianDentalAssociation_EmergencyDentistSearch.EmergencyDentistSearchID, Practice + ' ' + ISNULL(Street,'') + ' ' + City + ' ' + State + ' ' + PostCode as SearchAddressFormat, ISNULL(Street,'') + ' ' + City + ' ' + State + ' ' + PostCode as SearchAddressFormat2 FROM dbo.Form_AustralianDentalAssociation_EmergencyDentistSearch left outer join dbo.ADANSW_DEDGeoCodes on dbo.Form_AustralianDentalAssociation_EmergencyDentistSearch.EmergencyDentistSearchID = dbo.ADANSW_DEDGeoCodes.EmergencyDentistSearchID where agree = 1 and dbo.ADANSW_DEDGeoCodes.SearchAddressFormat is null order by dbo.Form_AustralianDentalAssociation_EmergencyDentistSearch.EmergencyDentistSearchID",
}
const configKentico = {
    user: config.user,
...
sql.connect(configKentico).then(async () => {
    ...
    request.on('row', async row => {
        ...
        //console.log(new_record.FormattedAddress);
...
getGeoCode = async (row,SearchAddressFormat) => {
    let new_record;
    let url = `https://maps.googleapis.com/maps/api/geocode/json?address=${encodeURIComponent(SearchAddressFormat)}&key=${config.api_key}`;
    try {
...
insertNewRecord = async (newrecord) => {
    ...
    request.query("INSERT INTO dbo.ADANSW_DEDGeoCodes (EmergencyDentistSearchID,SearchAddressFormat,GeoCode,Latitude,Longitude,FormattedAddress,PlaceID) VALUES (@EmergencyDentistSearchID,convert(nvarchar(max),HASHBYTES('MD5',@SearchAddressFormat),2),@GeoCode,@Latitude,@Longitude,@FormattedAddress,@PlaceID)", 
        (err, result) => {
        console.log("Inserted:",newrecord.EmergencyDentistSearchID, `(${newrecord.PlaceID})`)
    })
}
